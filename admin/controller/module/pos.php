...
class ControllerModulePos extends Controller {
	...
	public function modify_order($data) {
		...
		if ($action == 'insert') {
			...
			$this->db->query("INSERT INTO `" . DB_PREFIX . "order_product` SET order_id = '" . (int)$order_id . "', product_id = '" . (int)$data['product_id'] . "', name = '" . $this->db->escape($data['product']['name']) . "', model = '" . $this->db->escape($data['product']['model']) . "', quantity = '" . (int)$data['quantity'] . "', price = '" . (float)$data['product']['price'] . "', total = '" . ((float)$data['product']['price'] * (int)$data['quantity']) . "', tax = '" . $tax . "', reward = '" . $reward_points . "'");
